...
class Login_tes:#登录模块封装
    ...
    def login(self,suc,name,password):
        try:
            self.driber.element_wait(self.denglu).click()
            self.driber.element_wait(self.username).clear()
            self.driber.element_wait(self.username).send_keys(name)
            self.driber.element_wait(self.password).click()
            self.driber.element_wait(self.password).send_keys(password)
            self.driber.element_wait(self.sub).click()
            if suc=='1':
